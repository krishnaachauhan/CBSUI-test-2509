...
export const DenominationReceiptMetaData: CustomTableMetadataType = {
  ...
  fields: [
    ...
    },
    {
      ...
      dependentValue: ["DENO_VAL", "AVAIL_QTY", "PENDING_DENO"],
      onChange: async (
        currentFieldValue,
        rowData,
        dependentValues,
        ...
        const P_REMAIN = dependentValues?.[2] - Math.abs(CurrentValue);
        setDependentValue("P_REMAIN", P_REMAIN ?? 0);
      ...
      validation: async (
        ...
        const P_REMAIN = dependentValues?.[2] - Math.abs(currentFieldValue);
        ...
        } else if (currentFieldValue > 0 && P_REMAIN < 0) {
          const Btn = await MessageShow({
            messageTitle: "ValidationFailed",
            message: `Denomination ${currentFieldValue} should be less than or equal to Balance Amount.`,
            icon: "ERROR",
            buttonNames: ["Ok"],
          });
          if (Btn === "Ok") {
            updateCurrentField?.("");
            setDependentValue("DENO_AMOUNT", "0.00");
            setDependentValue("P_REMAIN", "0.00");
          }
        } else if (Boolean(CurrentValue) && remaning?.DENO_AMOUNT === 0) {
