      ...
      _fields: [
        ...
        },
        {
          ...
          shouldExclude(fieldData, dependentFieldsValues, formState) {
            ...
            const jointHolders = formState?.initialVal?.JOINT_GUARANTOR_DTL;
        ...
        },
        {
          ...
          accountTypeMetadata: {
            ...
            postValidationSetCrossFieldValues: async (
              ...
              if (
                currentField?.value &&
                !dependentFieldValues?.["JOINT_GUARANTOR_DTL.REF_BRANCH_CD"]
                  ?.value
              ) {
                let buttonName = await formState?.MessageBox({
          ...
          accountCodeMetadata: {
            ...
            postValidationSetCrossFieldValues: async (
              ...
              if (
                currentField?.value &&
                !dependentFieldsValues?.["JOINT_GUARANTOR_DTL.REF_ACCT_TYPE"]
                  ?.value
              ) {
                let buttonName = await formState?.MessageBox({
                  messageTitle: "ValidationFailed",
                ...
                currentField?.value &&
                dependentFieldsValues?.["JOINT_GUARANTOR_DTL.REF_BRANCH_CD"]
                  ?.value &&
                dependentFieldsValues?.["JOINT_GUARANTOR_DTL.REF_ACCT_TYPE"]
                  ?.value
              ) {
                  ...
                  REF_ACCT_CD: returnVal
                    ? {
                        value: utilFunction?.getPadAccountNumber(
                          currentField?.value,
                          dependentFieldsValues?.[
                            "JOINT_GUARANTOR_DTL.REF_ACCT_TYPE"
                          ]?.optionData?.[0] ?? ""
                        ),
                        isFieldFocused: false,
                        ignoreUpdate: true,
        ...
        },
        {
          ...
          setValueOnDependentFieldsChange: (dependentFields) => {
            return dependentFields?.["JOINT_GUARANTOR_DTL.MEM_ACCT_CD"]?.value;
          },
          validationRun: "all",
          postValidationSetCrossFieldValues: (
            ...
            if (
              field?.value &&
              dependentFields?.["JOINT_GUARANTOR_DTL.COPY_ACCT_NO"]?.value &&
              field?.value ===
                dependentFields?.["JOINT_GUARANTOR_DTL.COPY_ACCT_NO"]?.value
            ) {
        ...
        },
        {
          ...
          setFieldLabel: (dependentValue) => {
            const custType =
              ...
              "";
            const gender =
              dependentValue?.["JOINT_GUARANTOR_DTL.GENDER"]?.value ?? "";
            if (Boolean(custType)) {
              if (
                custType === "I" &&
        ...
        },
        {
          ...
          postValidationSetCrossFieldValues: async (
            ...
            if (field?.value === "Y") {
              return {
                PIN_CODE: {
                  value: dependent?.["JOINT_GUARANTOR_DTL.PIN_CODE"]?.value,
                  ignoreUpdate: true,
                },
              };
            } else if (!dependent?.["JOINT_GUARANTOR_DTL.AREA_CD"]?.value) {
              return {
                PIN_CODE: {
                  value: "",
                  ignoreUpdate: true,
          ...
          postValidationSetCrossFieldValues: async (
            ...
            if (field?.value) {
              ...
              return {
                FLAG: {
                  value:
                    dependentFieldsValues?.["JOINT_GUARANTOR_DTL.FLAG"]
                      ?.value === "Y"
                      ? "Y"
                      : new Date(),
                },
