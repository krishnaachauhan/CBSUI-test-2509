...
const NomineeJointTab = () => {
  ...
  const UpdatedMetadata = useMemo(() => {
    const UpdateMeta = { ...nomineejoint_tab_metadata };
  ...
  const jointDtlData = useJointDetailByTabName("NOMINEE", AcctMSTState);
  ...
  const initialVal = useMemo(() => {
    ...
    return {
      JOINT_NOMINEE_DTL: AcctMSTState?.isFreshEntryctx
      ...
      <FormWrapper
        ...
        onFormButtonClickHandel={async (id, dependentFields) => {
          if (id.slice(id.indexOf(".") + 1) === "CRDT_WORTHINESS_ignoreField") {
            let acct_num =
              dependentFields?.["JOINT_NOMINEE_DTL.MEM_ACCT_CD"]?.value?.trim();
