...
const JointTab = () => {
  ...
  const UpdatedMetadata = useMemo(() => {
    const UpdateMeta = { ...joint_tab_metadata };
  ...
  const jointDtlData = useJointDetailByTabName("JOINT", AcctMSTState);
  ...
  const initialVal = useMemo(() => {
    ...
    return {
      JOINT_HOLDER_DTL: AcctMSTState?.isFreshEntryctx
      ...
      <FormWrapper
        ...
        onFormButtonClickHandel={async (id, dependentFields) => {
          if (id.slice(id.indexOf(".") + 1) === "CRDT_WORTHINESS_ignoreField") {
            let acct_num =
              dependentFields?.["JOINT_HOLDER_DTL.MEM_ACCT_CD"]?.value?.trim();
