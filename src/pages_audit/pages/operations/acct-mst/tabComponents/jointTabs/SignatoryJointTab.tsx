...
const SignatoryJointTab = () => {
  ...
  const UpdatedMetadata = useMemo(() => {
    const UpdateMeta = { ...signatoryjoint_tab_metadata };
  ...
  const jointDtlData = useJointDetailByTabName("SIGNATORY", AcctMSTState);
  const initialVal = useMemo(() => {
    ...
    return {
      JOINT_SIGNATORY_DTL: AcctMSTState?.isFreshEntryctx
      ...
      <FormWrapper
        ...
        onFormButtonClickHandel={async (id, dependentFields) => {
          if (id.slice(id.indexOf(".") + 1) === "CRDT_WORTHINESS_ignoreField") {
            let acct_num =
              dependentFields?.[
                "JOINT_SIGNATORY_DTL.MEM_ACCT_CD"
              ]?.value?.trim();
            ...
            if ((acct_num !== "" && isOpen) || (retrieveAcctNum && isOpen)) {
              ...
              setAccountNumberDetails({
                ...
                A_BRANCH_CD:
                  ...
                  "",
                A_ACCT_TYPE:
                  dependentFields?.["JOINT_SIGNATORY_DTL.MEM_ACCT_TYPE"]
                    ?.value ?? "",
                A_ACCT_CD:
