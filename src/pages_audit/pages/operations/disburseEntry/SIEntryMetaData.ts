    ...
    },
    {
      ...
      validate: (currentField, dependentField, formstate) => {
        const parsedDate = formstate.authState?.workingDate
          ? parse(formstate?.authState?.workingDate, "dd/MMM/yyyy", new Date())
          : "";
        const startDate = currentField?.value ? currentField?.value : null;
        if (!utilFunction?.isValidDate(startDate)) {
          return t("PleaseEnterValidDate");
        }
        if (startDate && parsedDate) {
          if (isBefore(startDate, parsedDate)) {
    ...
    },
    {
      ...
      validate: (currentField, dependentField) => {
        if (
          ...
          const validUptoDate = new Date(currentField.value);
          const startDate = new Date(dependentField.START_DT.value);
          if (
            isBefore(validUptoDate, startDate) ||
            isEqual(validUptoDate, startDate)
          ) {
